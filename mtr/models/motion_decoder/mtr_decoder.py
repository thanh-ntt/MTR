...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        # section "Dense future prediction for future interactions" (it's part of the Encoder, but fn is placed here)
    ...
    # WARNING: this fn is in decoder, but it's the Dense Future Prediction block in the encoder in paper's figures!
    ...
    # TODO: does this get_motion_query get motion query pair or just the static intention query?
    # intention_points = static intention points = I = (64, 2)
    # intention_query = MLP(PE(I)) = (64, 256) <= static intention query
    ...
    # WARNING: this is actually applying BOTH cross attention & self attention
    # refer to mtr/models/utils/transformer/transformer_decoder_layer.py:75
    def apply_cross_attention(self, kv_feature, kv_mask, kv_pos, query_content, query_embed, attention_layer,
        ...
        if not use_local_attn:
            ...
            # function definition: mtr/models/utils/transformer/transformer_decoder_layer.py:75
            query_feature = attention_layer(
                tgt=query_content,
                query_pos=query_embed,
                query_sine_embed=searching_query,
    ...
    def apply_transformer_decoder(self, center_objects_feature, center_objects_type, obj_feature, obj_mask, obj_pos, map_feature, map_mask, map_pos):
        ...
        base_map_idxs = None
        # initialize predicted endpoints (pred_waypoints) to original intention points generated by k-mean clustering
        # Then, at subsequent iterations, use the whole predicted waypoints
        # (as figure 3 illustrates that apply_dynamic_map_collection collect L=128 polylines)
        # TODO: confirm above, as mtr/models/motion_decoder/mtr_decoder.py:373 only slice first 2 elements in pred_trajs
        ...
        for layer_idx in range(self.num_decoder_layers):
            # BOTH Cross attention + Self-attention ---------------------------------- #
            # Self-attention block is nested within self.obj_decoder_layers and self.map_decoder_layers
            # self.apply_cross_attention actually contains both self/cross-attention blocks
            # The "Multi-Head Attention" block (2nd block) in figure 2
            #   => 2 separate cross-attention blocks
            ...
            # (end) BOTH Cross attention + Self-attention ---------------------------------- #
            ...
            # self.motion_vel_heads was set to none (seems like an experiment)
            ...
            # update
            # pred_waypoints = predicted endpoint only (not trajectory)
            # TODO: why last dimension of pred_waypoints takes 2 elements from pred_trajs?
            #  <= take the endpoint only (first element in pred_trajs)?
            #  <= X,Y coordinate in the same dimension of the tensor?
